...
func LoginTelex(c *gin.Context) {
	...
	if formattedMessage == "/start-mail" {
		...
		c.JSON(http.StatusBadRequest, gin.H{"status": "error", "message": "Login successful. However, you need to type a message to start email monitoring service.", "username": "Automated Email Service", "event_name": "Handling Emails"})
	}
}
